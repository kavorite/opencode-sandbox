...
|-------|------|---------|-------------|
| `network.mode` | string | `"block"` | `"block"` blocks all network access. `"observe"` routes HTTP/HTTPS through mitmproxy to capture method, host, and TLS SNI. |
| `network.allow` | string[] | `[]` | IP addresses that are allowed and won't be flagged as violations. |
| `network.allow_methods` | string[] | `["GET","HEAD","OPTIONS"]` | HTTP methods allowed through the mitmproxy sidecar (observe mode). |
| `network.allow_graphql_queries` | boolean | `true` | Allow GraphQL POST requests even when POST is not in allow_methods. |
...
2. In `permission.ask`, the command is run inside a warm Docker container via `docker exec`.
...
8. The original command is replaced with a no-op (`true`) — it already ran inside Docker.

## Network Observation (observe mode)

When `network.mode` is set to `"observe"`, a mitmproxy sidecar container is started on the same Docker network. Sandbox containers route HTTP/HTTPS through it via `HTTP_PROXY`/`HTTPS_PROXY` environment variables.
...
| Behavior | Severity |
|----------|----------|
| Network connections (block mode) | high |
| HTTP methods not in allow_methods | high |
...
- **Programs ignoring HTTP_PROXY.** Applications with hardcoded connections bypass mitmproxy.
- **Cert pinning.** Applications using certificate pinning are impervious to MITM.
